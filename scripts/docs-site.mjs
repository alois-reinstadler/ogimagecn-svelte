...
export async function generateSite({ output, origin, basePath, library, components }) {
  ...
  const nav = (active) => `
    <header class="nav"><div class="wrap nav-inner">
      <a class="brand" href="${A("")}"><span class="dot"></span> ogimagecn-svelte</a>
      <nav class="nav-links">
  ...
  const code = (source, label) => `
    <div class="codeblock">
      ${label ? `<span class="copy" data-copy="${esc(source)}">copy</span>` : ""}
...
</head><body>${nav(path === "/" ? "home" : path.startsWith("/components") ? "components" : path.startsWith("/docs") ? "docs" : "")}
<main class="wrap">${body}</main>${footer()}
<script>
  ...
  // ---- render all component previews + collect card data ----
  ...
  const home = layout({
    ...
    <section class="hero">
      ...
      <h1>Open Graph images, without React.</h1>
      ...
      <div class="cta-row">
        <a class="btn primary" href="${A("docs/getting-started.html")}">Get started</a>
        <a class="btn" href="${A("components/")}">Browse components</a>
      </div>
      ${code("pnpm add ogimagecn-svelte", "install")}
    </section>
    <section class="section">
      ...
      <div class="grid cols-3">
        <div class="feature"><h3>Server-rendered, deterministic</h3><p>SVG and PNG output is byte-stable for the same inputs &mdash; ideal for build-time and edge rendering.</p></div>
        <div class="feature"><h3>Svelte 5 runes &amp; snippets</h3><p>Props are typed and use runes. Compose with an <code>overlay</code> snippet; never author React-shaped objects.</p></div>
        <div class="feature"><h3>shadcn-svelte install</h3><p>Own the source via the shadcn-svelte CLI, or import the typed components straight from the package.</p></div>
        <div class="feature"><h3>Satori constraints enforced</h3><p>Strict mode turns renderer warnings into errors so broken layouts fail loudly, not silently.</p></div>
        <div class="feature"><h3>Bundled fonts</h3><p>Licensed Noto Sans + CJK fixtures ship with the package, with helpers to load any other font.</p></div>
        <div class="feature"><h3>${cards.length} ready components</h3><p>From minimal cards to product, profile, and registry showcases &mdash; all inside Satori's CSS subset.</p></div>
      </div>
    </section>
    <section class="section">
      ...
      <div class="gallery">
        ${cards.slice(0, 6).map((c) => `<a class="card" href="${A(`components/${c.name}.html`)}">
          <img src="${A(`previews/${c.name}.png`)}" width="600" height="315" loading="lazy" alt="${esc(c.title)} Open Graph image">
          <div class="body"><h3>${esc(c.title)}</h3><p>${esc(c.description)}</p></div>
        </a>`).join("\n")}
      </div>
      <p style="margin-top:24px"><a class="btn" href="${A("components/")}">View all ${cards.length} components &rarr;</a></p>
    </section>`
  });
  ...
  const gallery = layout({
    ...
    <section class="section" style="padding-top:48px">
      ...
      <div class="gallery">
        ${cards.map((c) => `<a class="card" href="${A(`components/${c.name}.html`)}">
          <img src="${A(`previews/${c.name}.png`)}" width="600" height="315" loading="lazy" alt="${esc(c.title)} Open Graph image">
          <div class="body"><h3>${esc(c.title)}</h3><p>${esc(c.description)}</p></div>
        </a>`).join("\n")}
      </div>
  ...
  // ---- per-component pages (data-driven from registry defaults) ----
  ...
  for (const c of cards) {
    ...
    const page = layout({
      ...
      body: `
      <section class="section" style="padding-top:48px">
        <p><a href="${A("components/")}">&larr; All components</a></p>
        <h2>${esc(c.title)}</h2>
        ...
        <table class="props"><thead><tr><th>Prop</th><th>Default</th></tr></thead><tbody>${propRows}</tbody></table>
        <h3>shadcn-svelte</h3>
  ...
  const docsPage = ({ title, description, key, body }) => layout({
    ...
    body: `<div class="docs-layout">${docsNav(key)}<div class="docs-content">${body}</div></div>`
  });
  ...
  await renderPage("docs/index.html", docsPage({
    ...
    body: `
      ...
      <div class="grid cols-2" style="margin-top:24px">
        <a class="feature" href="${A("docs/getting-started.html")}"><h3>Getting started</h3><p>Install and render your first image in a SvelteKit endpoint.</p></a>
        <a class="feature" href="${A("docs/api.html")}"><h3>API reference</h3><p><code>renderSvg</code>, <code>renderImage</code>, <code>renderPng</code>, <code>loadFont</code> and every option.</p></a>
        <a class="feature" href="${A("docs/composition.html")}"><h3>Composition</h3><p>Use the <code>overlay</code> snippet to compose Satori-compatible layouts.</p></a>
        <a class="feature" href="${A("docs/fonts.html")}"><h3>Fonts &amp; emoji</h3><p>Load custom fonts and handle missing glyphs deterministically.</p></a>
        <a class="feature" href="${A("docs/satori.html")}"><h3>Satori constraints</h3><p>What the CSS subset allows, and why strict mode matters.</p></a>
        <a class="feature" href="${A("components/")}"><h3>Components</h3><p>Browse all ${cards.length} components with props and snippets.</p></a>
      </div>`
  }));
...
const { png } = await renderImage(Simple, props);`)}
      ...
      ${code(`// src/routes/og/[slug]/+server.ts
...
export async function GET() {
  ...
  await renderPage("docs/api.html", docsPage({
    ...
    body: `
      ...
      <h2>RenderOptions</h2>
      <table class="props"><thead><tr><th>Option</th><th>Type</th><th>Default</th></tr></thead><tbody>
...
<Simple title="Composed in Svelte" overlay={badge} />`)}
  ...
  await renderPage("docs/fonts.html", docsPage({
    ...
    body: `
      ...
      ${code(`import { loadFont, renderSvg } from 'ogimagecn-svelte';
...
await renderSvg(Simple, { title: 'नमस्ते' }, { fonts: [devanagari] });`)}
  ...
  await renderPage("404.html", layout({
    ...
    body: `<section class="hero"><h1>404</h1><p class="lead">That page does not exist. <a href="${A("")}">Go home &rarr;</a></p></section>`
  }));
